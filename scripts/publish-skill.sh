...
SKILL_NAME="douyin-fast-parse"
WORK_DIR="$(mktemp -d /tmp/douyin-fast-parse-skill-XXXXXX)"
trap 'rm -rf "$WORK_DIR"' EXIT
TS="$(date +%Y%m%d%H%M%S)"
RELEASE_PATH="douyin-fast-parse/release"
...
"$ROOT_DIR/scripts/upload-file.sh" --file "$ROOT_DIR/skills/douyin-fast-parse/scripts/run.sh" --name "${SKILL_NAME}.sh" >/dev/null
